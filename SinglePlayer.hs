...
initializeOnePlayer game | length(players game) == 0 = game{players = initializePlayers 1}
                         | length(players game) == 2 = game{players = [((players game) !! 0)]}
                         | otherwise = game
...
spRender game = pictures
   ...
   ++
   [
      mkTitles (isDead player) (highScore player) (score player) (lives player) (plColor player) | player <- (players game)
   ])
   ...
   where
    ...
    mkTitles :: Bool -> Float -> Float -> Float -> Color -> Picture
    mkTitles False hs s l col = pictures
     [
    ...
    mkTitles True _ _ _ _ = pictures [ blank ]

    -- | Display the number of player's lives
    showLives :: Float -> Color -> Picture
    showLives n col = pictures [scale (0.8) (0.8) (mkShip False False col ((-gWidth game)*(0.57) + 70 + x*40,(-gHeight game)*(0.55) + 32) 270) | x <- [1..n] ]